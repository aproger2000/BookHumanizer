"""
Chapter Editor v4.4.0 — с гибкой настройкой через config.py
"""
import json
import os
import re
...
# ========== Функции для модели и признаков ==========
...
# ========== Пост-обработка (с параметрами из config) ==========
def post_process(text: str, logs: list = None) -> str:
    ...
    if random.random() < config.PROB_INSERT_PARTICLES:
    ...
    for op in ops:
        if op == 'synonyms':
            ...
            for word in words:
                ...
                if clean.lower() in SYNONYMS and random.random() < config.SYNONYM_REPLACE_PROB:
            ...
            for sent in sentences:
                if len(sent.split()) > 5 and random.random() < config.INSERTION_PROB:
                    words = sent.split()
            ...
            swapped = 0
            for sent in sentences:
                if len(sent.split()) > 4 and random.random() < config.SWAP_FIRST_PROB:
                    words = sent.split()
            ...
            inserted_interj = 0
            for sent in sentences:
                if re.match(r'^[—"«]', sent) and random.random() < config.INTERJECTION_PROB:
            ...
            def replace_direct_indirect(text):
                pattern = re.compile(r'—\s*(.+?)\s*,\s*—\s*(сказал|сказала|произнёс|произнесла|ответил|ответила|спросил|спросила|пробормотал|пробормотала|крикнул|крикнула|прошептал|прошептала)\s+([а-яА-ЯёЁ]+)\.?')
                def repl(m):
            ...
            adverbs = ['вчера', 'сегодня', 'завтра', 'утром', 'вечером', 'ночью', 'днём', 'сейчас', 'тогда', 'потом', 'сначала', 'наконец']
            sentences = re.split(r'(?<=[.!?])\s+', text)
            new_sentences = []
            inverted = 0
            for sent in sentences:
            ...
            text = '. '.join(new_sentences)
            if inverted:
                logs.append(f"  - инверсий: {inverted}")
            ...
            for sent in sentences:
                if len(sent.split()) > 3 and random.random() < config.INSERT_PARTICLES_PROB:
                    words = sent.split()
